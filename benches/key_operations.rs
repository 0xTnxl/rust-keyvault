...
fn bench_key_generation(c: &mut Criterion) {
    ...
    group.bench_function("aes256_gcm", |b| {
        b.iter(|| {
            SecretKey::generate(black_box(Algorithm::Aes256Gcm))
        });
    });
    ...
    group.bench_function("chacha20_poly1305", |b| {
        b.iter(|| {
            SecretKey::generate(black_box(Algorithm::ChaCha20Poly1305))
        });
    });
    ...
    group.bench_function("xchacha20_poly1305", |b| {
        b.iter(|| {
            SecretKey::generate(black_box(Algorithm::XChaCha20Poly1305))
        });
    });
