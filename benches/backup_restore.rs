            ...
            |b, &count| {
                b.iter_batched(
                    ...
                    |(backup, restore_dir, _temp_dir)| {
                        ...
                        let mut new_store =
                            FileStore::new(restore_dir.path(), StorageConfig::default())
                                .expect("Failed to create new FileStore");
                        new_store
                            .restore(black_box(&backup), black_box(password))
...
fn bench_backup_restore_roundtrip(c: &mut Criterion) {
    ...
    group.bench_function("roundtrip_10_keys", |b| {
        b.iter_batched(
            ...
            |(mut store, _temp_dir)| {
                let backup = store
                    .backup(black_box(password), black_box(BackupConfig::default()))
                    .expect("Failed to create backup");
