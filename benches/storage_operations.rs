...
use rust_keyvault::storage::{MemoryStore, FileStore, KeyStore, StorageConfig};
...
fn bench_memory_store(c: &mut Criterion) {
    ...
    group.bench_function("store", |b| {
        ...
        b.iter(|| {
            store.store(black_box(key.clone()))
    ...
    group.bench_function("retrieve", |b| {
        ...
        b.iter(|| {
            store.retrieve(black_box(&key_id))
