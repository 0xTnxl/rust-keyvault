...
fn bench_key_import(c: &mut Criterion) {
    ...
    group.bench_function("import_default", |b| {
        b.iter(|| exported.decrypt(black_box(password)));
    });
...
fn bench_json_serialization(c: &mut Criterion) {
    ...
    group.bench_function("to_json", |b| {
        b.iter(|| exported.to_json().expect("Failed to serialize"));
    });

    // Deserialize from JSON
    let json_data = exported.to_json().expect("Failed to serialize");
    group.bench_function("from_json", |b| {
        b.iter(|| ExportedKey::from_json(black_box(&json_data)).expect("Failed to deserialize"));
    });
