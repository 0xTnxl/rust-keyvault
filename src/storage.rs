...
use crate::audit::{AuditLogger, AuditEvent, AuditLogEntry, NoOpLogger};
...
impl std::fmt::Debug for PersistedKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PersistedKey")
            ...
            .field("encrypted_key", &format!("[REDACTED {} bytes]", self.encrypted_key.len()))
            .finish()
    }
}
...
impl KeyStore for MemoryStore {
    fn store(&mut self, key: VersionedKey) -> Result<()> {
        ...
        let mut keys = self.keys.write().map_err(|_| crate::Error::storage("lock_acquire", "lock poisoned"))?;
        keys.insert(key_id, key);
        Ok(())
    }

    fn retrieve(&self, id: &KeyId) -> Result<VersionedKey> {
        let keys = self.keys.read().map_err(|_| crate::Error::storage("lock_acquire", "lock poisoned"))?;
        keys.get(id)
            .cloned()
    ...
    fn update_metadata(&mut self, id: &KeyId, metadata: KeyMetadata) -> Result<()> {
        ...
        if let Some(versioned_key) = keys.get_mut(id) {
            ...
            Err(crate::Error::storage("update_metadata", &format!("key not found: {id:?}")))
        }
    }
    ...
    fn get_latest_key(&self, id: &KeyId) -> Result<VersionedKey> {
        ...
        versions
            ...
            .ok_or_else(|| crate::Error::storage("get_latest_key", &format!("no active key found for: {id:?}")))
    }
}
...
impl KeyLifeCycle for MemoryStore {
    fn deprecate_key(&mut self, id: &KeyId) -> Result<()> {
        ...
        if let Some(key) = keys.get_mut(id) {
            ...
            Err(crate::Error::storage("deprecate_key", &format!("key not found: {id:?}")))
        }
    }
    ...
    fn revoke_key(&mut self, id: &KeyId) -> Result<()> {
        ...
        if let Some(key) = keys.get_mut(id) {
            ...
            Err(crate::Error::storage("revoke_key", &format!("key not found: {id:?}")))
        }
    }
...
impl std::fmt::Debug for FileStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FileStore")
            ...
            .field("master_key", &if self.master_key.is_some() { "[SET]" } else { "[NOT SET]" })
...
impl FileStore {
    ...
    pub fn rekey(&mut self, new_password: &[u8]) -> Result<()> {
        if !self.config.encrypted {
            return Err(crate::Error::storage("rekey", "encryption not enabled in config"));
        }

        // Ensure we have a current master key
        if self.master_key.is_none() {
            return Err(crate::Error::storage("rekey", "store is locked - cannot rekey"));
        }
        ...
        let salt = &self.vault_metadata
            .as_ref()
            .ok_or_else(|| crate::Error::storage("rekey", "vault metadata not initialized"))?
            .salt;
        
        // Derive new master key with the config
        let new_master_key = Self::derive_master_key(new_password, salt, &self.config.argon2_config)?;
    ...
    pub fn set_master_key(&mut self, key: crate::key::SecretKey) -> Result<()> {
        if !self.config.encrypted {
            return Err(crate::Error::storage("set_master_key", "encryption not enabled in config"));
        }
    ...
    fn serialize_key(&self, key: &VersionedKey) -> Result<Vec<u8>> {
        ...
        let encrypted_key = if self.config.encrypted {
            if let Some(master_key) = &self.master_key {
                ...
                return Err(crate::Error::storage("serialize_key", "encryption enabled but no master key set"));
            }
        } else {
            key_bytes
        };
        ...
        serde_json::to_vec(&persisted)
            .map_err(|e| crate::Error::storage("serialize_key", &format!("serialization failed: {e}"))) 
    }
    ...
    fn deserialize_key(&self, data: &[u8]) -> Result<VersionedKey> {
        let persisted: PersistedKey = serde_json::from_slice(data)
            .map_err(|e| crate::Error::storage("deserialize_key", &format!("deserialization failed: {e}")))?;
        ...
        let key_bytes = if self.config.encrypted {
            if let Some(master_key) = &self.master_key {
                ...
                if persisted.encrypted_key.len() < nonce_size {
                    return Err(crate::Error::storage("deserialize_key", "encrypted key too short - corrupted data"));
                }
                ...
                return Err(crate::Error::storage("deserialize_key", "encrypted key but no master key available"));
            }
        } else {
    ...
    pub fn init_with_password(&mut self, password: &[u8]) -> Result<()> {
        if !self.config.encrypted {
            return Err(crate::Error::storage("init_with_password", "encryption not enabled in config"));
        }
        
        let salt = &self.vault_metadata
            .as_ref()
            .ok_or_else(|| crate::Error::storage("init_with_password", "vault metadata not initialized"))?
            .salt;
    ...
    pub fn derive_master_key(
        ...
        ).map_err(|e| crate::Error::crypto("argon2_config", &format!("invalid Argon2 params: {}", e)))?;
        ...
        argon2.hash_password_into(password, salt, &mut key_bytes)
            .map_err(|e| crate::Error::crypto("argon2_hash", &format!("Argon2 derivation failed: {}", e)))?;
    ...
    fn load_metadata(&mut self) -> Result<()> {
        ...
        if metadata_path.exists() {
            ...
            let metadata: VaultMetadata = serde_json::from_slice(&data)
                .map_err(|e| crate::Error::storage("load_vault_metadata", &format!("failed to parse vault metadata: {}", e)))?;
            self.vault_metadata = Some(metadata);
        } else {
            let metadata = VaultMetadata::new()?;
            let data = serde_json::to_vec_pretty(&metadata)
                .map_err(|e| crate::Error::storage("load_vault_metadata", &format!("failed to serialize vault metadata: {}", e)))?;
    ...
    pub fn export_key(&mut self, id: &KeyId, password: &[u8]) -> Result<crate::export::ExportedKey> {
    ...
    pub fn import_key(&mut self, exported: &crate::export::ExportedKey, password: &[u8]) -> Result<KeyId> {
        // Decrypt the key
    ...
    pub fn backup(&mut self, password: &[u8], config: crate::backup::BackupConfig) -> Result<crate::backup::VaultBackup> {
        ...
        let created_at = self.vault_metadata.as_ref()
            .map(|m| m.created_at)
    ...
    pub fn restore(&mut self, backup: &crate::backup::VaultBackup, password: &[u8]) -> Result<usize> {
        // Decrypt the backup
...
impl KeyStore for FileStore {
    ...
    fn retrieve(&self, id: &KeyId) -> Result<VersionedKey> {
        ...
        if !key_path.exists() {
            return Err(crate::Error::storage("retrieve", &format!("key file not found: {id:?}")));
        }
    ...
    fn delete(&mut self, id: &KeyId) -> Result<()> {
        ...
        self.keys.remove(id)
            .ok_or_else(|| crate::Error::storage("remove_from_cache", &format!("key not found: {id:?}")))?;
    ...
    fn get_key_versions(&self, id: &KeyId) -> Result<Vec<VersionedKey>> {
        ...
        if versions.is_empty() {
            return Err(crate::Error::storage("sort_by_version_number", &format!("no versions found for key: {id:?}")));
        }
    ...
    fn get_latest_key(&self, id: &KeyId) -> Result<VersionedKey> {
        ...
        versions
            ...
            .filter(|k| matches!(k.metadata.state, KeyState::Active | KeyState::Rotating))
            .max_by_key(|k| k.metadata.version)
            .ok_or_else(|| crate::Error::storage("find_active_or_rotating_key", &format!("no active key found for: {id:?}")))
    }
}
...
mod tests {
    ...
    fn test_custom_argon2_config() {
        ...
        let secret_key = SecretKey::from_bytes(vec![0x77; 32], Algorithm::ChaCha20Poly1305).unwrap();
        let metadata = KeyMetadata {
            id: key_id.clone(),
            base_id: key_id.clone(),
            algorithm: Algorithm::ChaCha20Poly1305,
    ...
    fn test_memory_store_lifecycle() {
        ...
        let secret_key = SecretKey::from_bytes(vec![0x55; 32], Algorithm::ChaCha20Poly1305).unwrap();
        let metadata = KeyMetadata {
            id: key_id.clone(),
            base_id: key_id.clone(),
            algorithm: Algorithm::ChaCha20Poly1305,
    ...
    fn test_audit_logging() {
        use tempfile::tempdir;
        use crate::{Algorithm, key::SecretKey, audit::MemoryAuditLogger};
        use std::time::SystemTime;
        ...
        let secret_key = SecretKey::from_bytes(vec![0x88; 32], Algorithm::ChaCha20Poly1305).unwrap();
        let metadata = KeyMetadata {
            id: key_id.clone(),
            base_id: key_id.clone(),
            algorithm: Algorithm::ChaCha20Poly1305,
    ...
    fn test_safe_debug_implementations() {
        ...
        let _secret_key = SecretKey::from_bytes(vec![0x11; 32], Algorithm::ChaCha20Poly1305).unwrap();
    ...
    fn test_file_store_backup_restore() {
        ...
        let secret_key1 = SecretKey::from_bytes(vec![0x11; 32], Algorithm::ChaCha20Poly1305).unwrap();
        ...
        store.store(VersionedKey { key: secret_key1, metadata: metadata1 }).unwrap();
        
        let key_id2 = KeyId::from_bytes([2; 16]);
        let secret_key2 = SecretKey::from_bytes(vec![0x22; 32], Algorithm::XChaCha20Poly1305).unwrap();
        ...
        store.store(VersionedKey { key: secret_key2, metadata: metadata2 }).unwrap();
