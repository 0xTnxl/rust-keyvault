...
impl VaultBackup {
    ...
    pub fn new(backup_data: &BackupData, password: &[u8], config: &BackupConfig) -> Result<Self> {
        ...
        let data_to_encrypt = if config.compress {
            ...
            encoder.write_all(&serialized)
                .map_err(|e| Error::storage("compress_backup", &format!("compression failed: {}", e)))?;
            encoder.finish()
                .map_err(|e| Error::storage("compress_backup", &format!("compression failed: {}", e)))?
        } else {
            serialized
        ...
        argon2.hash_password_into(password, &salt, &mut derived_key)
            .map_err(|e| Error::crypto("derive_backup_key", &format!("Argon2 error: {}", e)))?;
        
        // Create a SecretKey for encryption
        let encryption_algorithm = Algorithm::XChaCha20Poly1305;
        let wrapping_key = crate::key::SecretKey::from_bytes(
            derived_key.clone(),
    ...
    pub fn decrypt(&self, password: &[u8]) -> Result<BackupData> {
        ...
        let argon2 = Argon2::new(Argon2Algo::Argon2id, Version::V0x13, params);
        let mut derived_key = vec![0u8; 32];
        argon2.hash_password_into(password, &self.salt, &mut derived_key)
            .map_err(|e| Error::crypto("derive_backup_key", &format!("Argon2 error: {}", e)))?;
        
        // Verify HMAC
        let calculated_hmac = Self::calculate_hmac(&self.encrypted_data, &derived_key)?;
        if calculated_hmac != self.metadata.checksum {
            return Err(Error::crypto("verify_backup_hmac", "HMAC verification failed - backup may be corrupted"));
        }
        ...
        let nonce_size = match self.encryption_algorithm {
            ...
            _ => return Err(Error::crypto("unsupported_algorithm", "unsupported encryption algorithm for backup")),
        };
        ...
        let decompressed = if self.metadata.compressed {
            ...
            decoder.read_to_end(&mut result)
                .map_err(|e| Error::storage("decompress_backup", &format!("decompression failed: {}", e)))?;
            result
        } else {
            decrypted
        };
        ...
        serde_json::from_slice(&decompressed)
            .map_err(|e| Error::storage("deserialize_backup", &format!("deserialization failed: {}", e)))
    }
    ...
    pub fn to_json(&self) -> Result<String> {
        serde_json::to_string_pretty(self)
            .map_err(|e| Error::storage("serialize_backup", &format!("serialization failed: {}", e)))
    }
...
mod tests {
    use super::*;
    use crate::{KeyId, KeyMetadata, KeyState, key::SecretKey, export::ExportedKey};
    ...
    fn test_backup_compression() {
        ...
        let backup_compressed = VaultBackup::new(&backup_data, password, &config_compressed).unwrap();
        ...
        let backup_uncompressed = VaultBackup::new(&backup_data, password, &config_uncompressed).unwrap();
